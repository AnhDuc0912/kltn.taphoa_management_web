...
def embed_text_clip_512(text: str) -> list[float]:
    # chuáº©n hoÃ¡ nháº¹ Ä‘á»ƒ á»•n Ä‘á»‹nh (Ä‘Ã£ cÃ³ vn_norm cho search, nhÆ°ng embed giá»¯ nguyÃªn ná»™i dung)
    t = (text or "").strip()
    if not t:
        return [0.0]*512
...
MD_MAX_RPM = float(os.getenv("MD_MAX_RPM", "60"))  # ~60 req/phÃºt
MD_MIN_INTERVAL = max(1.0 / MD_MAX_RPM, 0.016)     # giÃ£n cÃ¡ch giá»¯a 2 requests

VI_SYSTEM = "Báº¡n lÃ  biÃªn táº­p viÃªn viáº¿t mÃ´ táº£ sáº£n pháº©m, CHá»ˆ dÃ¹ng tiáº¿ng Viá»‡t."
PROMPT_SEARCH = (
  "Viáº¿t 1â€“2 cÃ¢u caption bÃ¡n hÃ ng, khÃ¡ch quan, tiáº¿ng Viá»‡t. "
  "Náº¿u Ä‘á»c Ä‘Æ°á»£c tÃªn thÆ°Æ¡ng hiá»‡u/loáº¡i trÃªn bao bÃ¬ thÃ¬ ghi CHÃNH XÃC. "
  "KhÃ´ng thÃªm emoji hay kÃ½ hiá»‡u."
)
PROMPT_SEO = (
  "Viáº¿t mÃ´ táº£ 3â€“5 cÃ¢u báº±ng tiáº¿ng Viá»‡t cho trang sáº£n pháº©m. "
  "Nháº¥n máº¡nh cÃ´ng dá»¥ng/Ä‘áº·c Ä‘iá»ƒm ná»•i báº­t; náº¿u tháº¥y thÆ°Æ¡ng hiá»‡u/biáº¿n thá»ƒ/kÃ­ch cá»¡ thÃ¬ ghi CHÃNH XÃC."
)

VI_CHARS  = r"[ÄƒÃ¢Ä‘ÃªÃ´Æ¡Æ°Ã¡Ã áº£Ã£áº¡áº¥áº§áº©áº«áº­áº¯áº±áº³áºµáº·Ã©Ã¨áº»áº½áº¹áº¿á»á»ƒá»…á»‡Ã­Ã¬á»‰Ä©á»‹Ã³Ã²á»Ãµá»á»‘á»“á»•á»—á»™á»›á»á»Ÿá»¡á»£ÃºÃ¹á»§Å©á»¥á»©á»«á»­á»¯á»±Ã½á»³á»·á»¹á»µ]"


def _prepare_image_for_api(img_bytes: bytes):
    """Resize + convert sang JPEG (â‰¤1280px, q=85) Ä‘á»ƒ payload gá»n, trÃ¡nh 422."""
    try:
...
def _mime_from_path(path: str) -> str:
    """
    ÄoÃ¡n MIME tá»« Ä‘Æ°á»ng dáº«n file; fallback 'image/jpeg' náº¿u khÃ´ng xÃ¡c Ä‘á»‹nh Ä‘Æ°á»£c.
    """
    # 1) Ä‘oÃ¡n theo mimetypes
    m = mimetypes.guess_type(path)[0]
    if m:
        return m
    # 2) fallback theo pháº§n má»Ÿ rá»™ng
...
def md_chat_vision(img_bytes: bytes, mime: str, prompt: str, ocr_text: str | None = None, max_tokens: int | None = None) -> str:
    if not MD_KEY:
        raise RuntimeError("Thiáº¿u MOONDREAM_API_KEY")

    # Chuáº©n hoÃ¡ áº£nh â†’ JPEG gá»n
    img_bytes, mime = _prepare_image_for_api(img_bytes)
    b64 = base64.b64encode(img_bytes).decode()

    # Prompt + OCR
    if ocr_text:
        prompt = f"{prompt}\nVÄƒn báº£n OCR: {ocr_text}\nNáº¿u phÃ¹ há»£p, dÃ¹ng Ä‘Ãºng thÆ°Æ¡ng hiá»‡u/loáº¡i tá»« OCR."
    user_text = prompt + "\nTráº£ lá»i 100% báº±ng tiáº¿ng Viá»‡t."

    headers = {
        "Authorization": f"Bearer {MD_KEY}",
        "X-Moondream-Auth": MD_KEY,             # an toÃ n hÆ¡n: gá»­i kÃ¨m header riÃªng cá»§a Moondream
    ...
    # Náº¿u váº«n ra tiáº¿ng Anh â†’ dá»‹ch láº§n cuá»‘i (text-only), LÆ¯U Ã: content cÅ©ng lÃ  LIST
    if not _is_vietnamese(out2):
        payload_tr = {
            "model": MD_MODEL,
            "messages": [
                {"role": "system", "content": [ {"type":"text","text":"Báº¡n lÃ  dá»‹ch giáº£ tiáº¿ng Viá»‡t."} ]},
                {"role": "user",   "content": [ {"type":"text","text": "Dá»‹ch sang tiáº¿ng Viá»‡t tá»± nhiÃªn:\n" + out2} ]}
            ],
            "max_tokens": 400
        }
...
def captions_autogen():
    """
    Táº¡o caption (style='search') + mÃ´ táº£ (style='seo') cho Táº¤T Cáº¢ áº£nh cÃ²n thiáº¿u,
    lÆ°u vÃ o sku_captions vá»›i needs_review=TRUE Ä‘á»ƒ dev duyá»‡t tay.
    Há»— trá»£ tham sá»‘: limit (máº·c Ä‘á»‹nh 200), offset (máº·c Ä‘á»‹nh 0)
    """
    limit  = int(request.form.get("limit", request.args.get("limit", 200)))
    offset = int(request.form.get("offset", request.args.get("offset", 0)))

    # Láº¥y danh sÃ¡ch áº£nh CHÆ¯A cÃ³ caption 'search' (dá»±a trÃªn image_path + sku_id)
    rows = q(f"""
    ...
    for (img_id, sku_id, img_path, ocr_text, sku_name, variant, size_tx, brand_name) in rows:
        try:
            ...
            # GhÃ©p OCR vÃ o prompt Ä‘á»ƒ tÄƒng chÃ­nh xÃ¡c brand/loáº¡i
            p_search = PROMPT_SEARCH + (f"\nVÄƒn báº£n OCR: {ocr_text}" if ocr_text else "")
            p_seo    = PROMPT_SEO    + (f"\nVÄƒn báº£n OCR: {ocr_text}" if ocr_text else "")

            # TÃ´n trá»ng rate limit (â‰ˆ60 rpm máº·c Ä‘á»‹nh)
            now = time.time()
            ...
            # Description (SEO) â€” thÃªm giÃ£n cÃ¡ch náº¿u cáº§n
            now = time.time()
            if now - last_call < MD_MIN_INTERVAL:
                time.sleep(MD_MIN_INTERVAL - (now - last_call))
            ...
            # LÆ°u vÃ o sku_captions (needs_review = TRUE)
            exec_sql("""
    ...
    # refresh corpus Ä‘á»ƒ search báº¯t ká»‹p (náº¿u dÃ¹ng corpus)
    try:
        exec_sql("SELECT refresh_sku_search_corpus()", returning=True)
    except:
        pass

    # Tráº£ UI: quay vá» trang SKUs vá»›i thÃ´ng bÃ¡o
...
def text_vec_backfill():
    ...
    for tid, text in rows:
        try:
            ...
            err += 1
    # KN: táº¡o/cháº¡y index HNSW 1 láº§n á»Ÿ DB (Ä‘Ã£ cÃ³ trong schema)
    flash(f"Backfill xong: {ok} ok, {err} lá»—i", "success" if err==0 else "warning")
    return redirect(url_for("skus"))
...
def import_csv_sample():
    ...
    sample.write("Háº£o Háº£o,MÃ¬ gÃ³i,MÃ¬ Háº£o Háº£o,TÃ´m chua cay,75g,8934567890123,/path/to/haohao_75g.jpg\n")
    sample.write("Omachi,MÃ¬ gÃ³i,MÃ¬ Omachi,BÃ² háº§m,80g,8934567890456,/path/to/omachi_bo_80g.jpg\n")
    sample.write("Vinamilk,Sá»¯a,Sá»¯a Vinamilk,KhÃ´ng Ä‘Æ°á»ng,180ml,8934567890789,/path/to/vinamilk_khongduong.jpg\n")
    sample.seek(0)
...
@app.get("/skus")
def skus():
    # lá»c theo brand/category/search
...
def sku_create():
    ...
    # Táº¡o sku_text chuáº©n hoÃ¡ Ä‘á»ƒ search sau nÃ y (vector cÃ³ thá»ƒ NULL)
...
def sku_update(sku_id):
    ...
    # cáº­p nháº­t sku_text chuáº©n hoÃ¡
    ...
    # náº¿u Ä‘Ã£ cÃ³ thÃ¬ update, khÃ´ng cÃ³ thÃ¬ insert
...
def sku_image_add(sku_id):
    ...
    # ðŸ”‘ Chá»‰ lÆ°u tÃªn file hoáº·c relative path
    db_path = fname  # hoáº·c "images/" + fname náº¿u muá»‘n cÃ³ folder con
    print(db_path)
...
def import_csv():
    ...
    for r in reader:
        ...
        # áº£nh (náº¿u cung cáº¥p Ä‘Æ°á»ng dáº«n cá»¥c bá»™)
        if image_file and os.path.exists(image_file):
            # --- trong sku_image_delete(): xÃ³a file an toÃ n ---
            row = q("SELECT image_path FROM sku_images WHERE id=%s", (img_id,), fetch="one")
            if row and row[0]:
                try:
                    fpath = row[0]
...
def captions_suggest():
    ...
    # required: sku_id, image_path (tÃªn file tÆ°Æ¡ng Ä‘á»‘i), caption_text, style, model_name, prompt_version
    exec_sql("""
...
def captions_label(caption_id):
    ...
    # sau khi duyá»‡t/sá»­a, refresh corpus Ä‘á»ƒ search báº¯t ká»‹p
    try:
        exec_sql("SELECT refresh_sku_search_corpus()", returning=True)
    except:
        pass
