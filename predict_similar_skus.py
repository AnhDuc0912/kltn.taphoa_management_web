...
def load_model(checkpoint_path="out-resnet101-model/finetuned_resnet101.pt", device="cuda", fine_tune=False, train_loader=None):
    ...
    if os.path.exists(checkpoint_path):
        ...
        try:
            checkpoint = torch.load(checkpoint_path, map_location=lambda storage, loc: storage.cuda(device.index) if torch.cuda.is_available() else storage)
...
def extract_embedding(model, image_paths, device=None, reduce_dim=False, n_components=256):
    ...
    try:
        ...
        for path in image_paths:
            ...
            if img is not None:  # Thêm kiểm tra ảnh hợp lệ
                img = infer_tf(img).unsqueeze(0).to(device)
                with torch.no_grad():
                    emb = model(img).cpu().numpy().astype("float32")
                    faiss.normalize_L2(emb)
                    if reduce_dim:
                        emb = reduce_embedding(emb, n_components)
                    else:
                        emb = emb[0].tolist()
...
def build_faiss_index(vectors, dim=512, m=64, nlist=None, nprobe=10):
    ...
    quantizer = faiss.IndexFlatL2(dim)  # Quay lại IndexFlatL2 như Colab để kiểm tra
...
class AllImagesDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        try:
            image = preprocess_image(img_path).convert('RGB')
            if self.transform:
...
if __name__ == "__main__":
    ...
    all_images_dataloader = DataLoader(all_images_dataset, batch_size=128, shuffle=False, num_workers=2)
    ...
    with torch.no_grad():
        for images, paths in all_images_dataloader:
            ...
            valid_paths = [paths[i] for i, img in enumerate(images) if img is not None]
            if len(valid_images) > 0:
                valid_images = torch.stack(valid_images).to(device)
                batch_embeddings = model(valid_images).cpu().numpy()
    ...
    # Search with uploaded image embedding (giả lập từ ảnh đầu tiên)
    k = 5
